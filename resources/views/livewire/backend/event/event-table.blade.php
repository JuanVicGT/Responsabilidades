        ...
        <x-mary-table :headers="$headers" :rows="$rows" with-pagination :sort-by="$sortBy" class="pb-4" striped
            ...
            @scope('cell_status', $row)
                ...
                @if ($row->status == 'cancelled')
                    <x-mary-badge value="{{ __(ucfirst($row->status)) }}" class="badge-error text-white dark:text-black" />
                @endif
                @if ($row->status == 'finished')
                    <x-mary-badge value="{{ __(ucfirst($row->status)) }}" class="badge-info text-white dark:text-black" />
                @endif
            @endscope
