<x-mary-step step="1" text="{{ __('General Data') }}">
    <section class="grid grid-cols-4 gap-4">
        <div class="col-span-2">
            <x-mary-input label="{{ __('Number') }}" wire:model="form_step1_number" autofocus required />
        </div>
        <div class="col-span-2">
            <x-mary-choices label="{{ __('Responsable') }}" debounce="500ms" wire:model="form_step1_id_responsible" single
                icon="o-user" no-result-text="{{ __('No results found.') }}" required class="max-h-12 external-choice"
                :options="$step1_option_users" searchable search-function="searchUsers">
                {{-- Item slot --}}
                @scope('item', $user)
