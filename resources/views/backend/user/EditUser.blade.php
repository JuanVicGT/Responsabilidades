<x-layouts.app>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                    <x-mary-select label="{{ __('Role') }}" icon="o-lock-closed" name="role" :options="$roles"
                        option-value="name" placeholder="{{ old('role', $current_role->name ?? __('Select a Role')) }}"
                        placeholder-value="{{ old('role', $current_role->name ?? null) }}" required />
    ...
    {{-- Reset Password Form --}}
    @if (isset($user->need_password_reset) && $user->need_password_reset)
        <x-mary-card title="{{ __('Reset Password') }}" separator>

            <x-mary-textarea type="text" readonly class="mb-2"
                rows="5">{{ $user->pendingPasswordResetRequest->description }}
            </x-mary-textarea>

            <div class="flex gap-4 justify-between">

                {{-- Decline Reset Password --}}
                <form method="POST" action="{{ route('user.refuse.password', $user->id) }}" x-data="{ submitButtonDisabled: false }"
                    x-on:submit="submitButtonDisabled = true" class="flex">
                    @csrf

                    <input type="hidden" name="id" value="{{ $user->id }}">

                    <x-mary-button type="submit" class="btn-error" x-bind:disabled="submitButtonDisabled">
                        <x-mary-loading class="text-primary" x-show="submitButtonDisabled" />
                        <div x-show="!submitButtonDisabled">
                            <svg class="inline w-5 h-5" xmlns="http://www.w3.org/2000/svg"
                            ...
                            </svg> {{ __('Refuse Request') }}
                        </div>
                    </x-mary-button>
                </form>

                {{-- Accept Reset Password --}}
                <form method="POST" action="{{ route('user.accept.password') }}" x-data="{ submitButtonDisabled: false }"
                    x-on:submit="submitButtonDisabled = true" class="flex">
                    @csrf
                    @method('patch')
