<x-layouts.app>
    ...
    @section('content-header')
        <x-mary-header title="{{ __('Edit Permission') }}" subtitle="{{ $permission->name }}">
            <x-slot:actions>
    ...
    <x-mary-card shadow>
        ...
        <x-mary-form method="POST" action="{{ route('permission.update') }}" x-data="{ submitButtonDisabled: false }"
            ...
            {{-- Submit Button --}}
            @if (auth()->user()->is_admin || auth()->user()->can('edit_permission'))
